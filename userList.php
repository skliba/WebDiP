...
if (isset($_SESSION['userType']) && $_SESSION['userType'] == 3) {
    ...
    $upit = "SELECT korisnik.*, vrstaStatusa.oznaka FROM korisnik,vrstaStatusa where vrstaStatusa.id = korisnik.vrstaStatusa_id and tipKorisnika_id not in ('3')";
    ...
    while ($red = $rezultat->fetch_array()) {
        ...
        $mail = $xml->createElement("mail", $red[1]);
        $mail = $podaci->appendChild($mail);


        $ime = $xml->createElement("ime", $red[2]);
        $ime = $podaci->appendChild($ime);

        $prezime = $xml->createElement("prezime", $red[3]);
        $prezime = $podaci->appendChild($prezime);

        $datumRodenja = $xml->createElement("datum_rodenja", $red[4]);
        $datumRodenja = $podaci->appendChild($datumRodenja);

        $adresa = $xml->createElement("adresa", $red[5]);
        $adresa = $podaci->appendChild($adresa);

        $grad = $xml->createElement("grad", $red[6]);
        $grad = $podaci->appendChild($grad);

        $spol = $xml->createElement("spol", $red[7]);
        $spol = $podaci->appendChild($spol);

        $username = $xml->createElement("username", $red[8]);
        $username = $podaci->appendChild($username);

        $password = $xml->createElement("password", $red[9]);
        $password = $podaci->appendChild($password);

        $status = $xml->createElement("status", $red[17]);
        $status = $podaci->appendChild($status);
    }
    ...
    $query = "insert into log values(default,'$log_user','$time','Korisnik $arrayzsz[0] je zatražio popis korisnika', 2)";
    $baza->updateDB($query);
} else {
    $time = getPomak();
    $id_user = $_GET['id'];
    ...
    $result = $baza->selectDB($query);
    $arrayzsz = $result->fetch_array();
    $query = "insert into log values(default,'$log_user','$time','Neuspjeli pokušaj pristupa korisnika $arrayzsz[0] na stranicu userList.php', 3)";
    $baza->updateDB($query);
...
<html>
    ...
    <body>
        <div id="wrapper">
            ...
            <div id="left">    
                <nav id="navigacija">
                    <ul>
                        ...
                        <li><a href="userList.php" id="curr">Popis korisnika</a></li>
            ...
            <section id="sadrzaj">
                <h3 id="popisKorisnikaTitle">Popis korisnika</h3>
                <div id="userList">
        ...
        <script type="text/javascript">
            ...
            myTable.append("<thead><tr><th>Ime</th><th>Prezime</th><th>E-mail</th><th>Username</th><th>Password</th><th>Adresa</th><th>Grad</th><th>Status</th><th>Detalji</th></tr></thead>");
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $(data).find('podaci').each(function () {
                            ...
                            red += "<td>" + $(this).find('grad').text() + "</td>";
                            red += "<td>" + $(this).find('status').text() + "</td>";
                            red += "<td><a href='detalji_korisnika.php?id=" + $(this).find('username').text() + "'>" + $(this).find('username').text() + "</a></td>";
